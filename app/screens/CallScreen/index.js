...
class CallScreen extends Component {
  constructor(props) {
    ...
    this._onCallAttendantTransferPress = this.onCallAttendantTransferPress.bind(
      this,
    );
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      (!this.state.call &&
        nextProps.call &&
        !(nextProps.call instanceof Promise)) ||
      (this.state.call &&
        nextProps.calls.hasOwnProperty(this.state.call.getId()))
    ) {
      ...
      const calls = Object.keys(nextProps.calls).map(
        key => nextProps.calls[key],
      );
      ...
      if (init) {
        ...
        CallAnimation.animateCallState(
          {...this.state, totalCalls: calls.length},
          call,
        );
  ...
  onCallAttendantTransferPress(destinationCall) {
    this.setState({isTransferModalVisible: false});
    this.props.onCallAttendantTransfer &&
      this.props.onCallAttendantTransfer(this.state.call, destinationCall);
  }

  onCallBlindTransferPress(value) {
    if (value.length > 0) {
      this.setState({isTransferModalVisible: false});
      this.props.onCallBlindTransfer &&
        this.props.onCallBlindTransfer(this.state.call, value);
    }
  }
  ...
  onCallRedirectSubmitPress(destination) {
    if (destination.length > 0) {
      this.setState({isRedirectModalVisible: false});
      this.props.onCallRedirect &&
        this.props.onCallRedirect(this.state.call, destination);
    }
  }

  onIncomingCallAnswer() {
    this.setState({incomingCall: null});
    this.props.onIncomingCallAnswer &&
      this.props.onIncomingCallAnswer(this.state.incomingCall);
  }

  onIncomingCallDecline() {
    this.setState({incomingCall: null});
    this.props.onIncomingCallDecline &&
      this.props.onIncomingCallDecline(this.state.incomingCall);
  }
  ...
  renderSimultaniousCalls() {
    ...
    for (const id in this.props.calls) {
      if (this.props.calls.hasOwnProperty(id) && id != activeCall.getId()) {
        // TODO: Check why type of ID is different
    ...
    return (
      <View
        style={{position: 'absolute', top: 5, width: this.state.screenWidth}}>
        {result}
      </View>
    );
  }
      ...
      <LinearGradient colors={['#2a5743', '#14456f']} style={cs.max}>
        <View style={cs.max}>
          ...
          <Animated.View
            ...
            <CallAvatar call={call} size={this.state.avatarHeight} />
          </Animated.View>
            ...
            actions={[
              {
                icon: 'call',
                text: 'Call',
                callback: this._onCallAddSubmitPress,
              },
            ]}
            ...
            actions={[
              {
                icon: 'blind-transfer',
                text: 'Redirect',
                callback: this._onCallRedirectSubmitPress,
              },
            ]}
            theme="dark"
          ...
          const doRoute = call => {
            ...
            for (const id in calls) {
              if (calls.hasOwnProperty(id)) {
                return dispatch(
                  Navigation.goAndReplace({name: 'call', call: calls[id]}),
                );
              }
            }
