...
export function goAndReplace(route) {
  return (dispatch, getState) => {
    ...
    const same =
      ['conversations', 'dialer', 'history', 'settings'].indexOf(route.name) !==
        -1 &&
  ...
  switch (action.type) {
    ...
    case NAVIGATE_BACK: {
      ...
      return {
        ...
        previous:
          newHistory.length > 0 ? newHistory[newHistory.length - 1] : {},
        history: newHistory,
        drawer: false,
      };
    }
    ...
    case NAVIGATE_RESET_WITH_STACK:
      return {
        ...
        previous:
          action.stack.length === 1
            ? {}
            : action.stack[action.stack.length - 2],
