...
export default class KeypadWithActions extends Component {
  ...
  renderActionKey(type, description, callback = noop) {
    let icon = null;
    const actionTouchableStyle =
      this.props.theme === 'dark' ? s.actionDarkTouchable : null;
    const actionTextStyle =
      this.props.theme === 'dark' ? s.actionDarkText : null;
    ...
    if (!this.state.actionSize) {
      return <View key={`action${type}`} style={s.defaultAction} />;
    }
  ...
  render() {
    ...
    if (this.props.actions.length === 3) {
      ...
      for (const action of this.props.actions) {
        actions.push(
          this.renderActionKey(action.icon, action.text, action.callback),
        );
      }
    }
    ...
    return (
      <View style={this.props.style}>
        ...
        />
        <View style={{flex: 0.02 * this.state.heightRatio}} />
        <View style={s.actionsWrapper}>{actions}</View>
        <View style={{flex: 0.02 * this.state.heightRatio}} />
      </View>
    );
  }
