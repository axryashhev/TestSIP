      ...
      <View {...props} key={digit} style={s.keyWrapper}>
        ...
        !this.state.keySize ? null : (
          <Touchable
            style={[
              s.keyTouchable,
              {width: this.state.keySize},
              this.props.keyStyle,
            ]}
            onPress={this.onKeyPress.bind(this, digit)}>
            <View style={s.keyDigitWrapper}>
              <Text style={[s.keyDigitText, this.props.keyTextStyle]}>
                {digit}
              </Text>
              <Text style={[s.keyLettersText, this.props.keyTextStyle]}>
                {letters}
              </Text>
            </View>
          </Touchable>
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (i !== keys.length - 1) {
        keypad.push(
          <View
            key={'split' + i}
            style={{flex: 0.006 * this.state.heightRatio}}
          />,
        );
      }
    }

    return <View style={this.props.style}>{keypad}</View>;
  }
}
