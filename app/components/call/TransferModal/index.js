...
export default class TransferModal extends Component {
  ...
  render() {
    if (
      Object.keys(this.props.calls).length == 1 ||
      this.state.isRedirectModalVisible
    ) {
      return (
        <DialerModal
          actions={[
            {
              icon: 'blind-transfer',
              text: 'Blind\nTransfer',
              callback: this._onBlindTransferPress,
            },
          ]}
          theme="dark"
          ...
          <View style={s.contentBackground}>
            <View style={s.titleContainer}>
              ...
              <TouchableOpacity onPress={this._onModalClosePress}>
                <Image
                  source={require('../../../assets/images/modal/close-icon.png')}
                />
              </TouchableOpacity>
            </View>
